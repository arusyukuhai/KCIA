...
const MODEL_LEN: usize = 4096;
const POP_SIZE: usize = 1024;
const ELITE: usize = 32;
const N_GEN: usize = 5000;
...
impl Dataset {
    fn build() -> Self {
        ...
        for _ in 0..1 {
            ...
            for i in 0..VEC_LEN {
                let v = -3.0 + 40.0 * i as f64 / VEC_LEN as f64;
                x.push(Complex::new(v, 0.0));
                one.push(Complex::new(1.0, 0.0));
            }

            let target: Vec<_> = x.iter().map(|z| (z).sin()).collect();
...
struct Evaluator {
    ...
    // 追加: (ノードのシグネチャ, ターゲットのインデックス) -> 相関係数
    corr_cache: DashMap<(Sig, usize), f64>,
}
...
impl Evaluator {
    fn new() -> Self {
        Self {
            ...
            corr_cache: DashMap::with_capacity(NODE_CACHE_CAP), // 同程度の容量を確保
        }
    }

    fn reset_node_cache(&self) {
        self.node_cache.clear();
        // node_cacheを消すなら、それに基づいた相関キャッシュも消すのが安全
...
fn score(pred: &[Complex<f64>], target: &[Complex<f64>]) -> f64 {
    ...
    let shape = {
        let re_shape =
            (chatterjee(&p[0], &t[0]) + chatterjee(&t[0], &p[0]) + pearson(&p[0], &t[0]).abs())
                / 3.0;
        let im_shape =
            (chatterjee(&p[1], &t[1]) + chatterjee(&t[1], &p[1]) + pearson(&p[1], &t[1]).abs())
                / 3.0;
        let norm_shape =
            (chatterjee(&p[2], &t[2]) + chatterjee(&t[2], &p[2]) + pearson(&p[2], &t[2]).abs())
                / 3.0;
        let arg_shape =
            (chatterjee(&p[3], &t[3]) + chatterjee(&t[3], &p[3]) + pearson(&p[3], &t[3]).abs())
                / 3.0;
...
fn eval(g: &Genome, ds: &Dataset, ev: &Evaluator) -> (f64, f64) {
    ...
    for ds_idx in 0..ds.n {
        ...
        let t_re: Vec<f64> = target.iter().map(|c| c.re).collect();

        let mut sum_corr = 0.0;
        let mut final_sig = 0;

        // executeの中身をインライン化、または拡張してキャッシュを利用
        let mut nodes: Vec<Option<(Sig, Vec<Complex<f64>>)>> = vec![None; N_INPUTS + MODEL_LEN];
        for i in 0..N_INPUTS {
            nodes[i] = Some((i as u64, ds.inputs[ds_idx][i].clone()));
        }
        ...
        for &abs in &active {
            ...
            let i = abs - N_INPUTS;
            let (s1, ref v1) = nodes[g.conn[i][0] as usize].as_ref().unwrap();
            let (s2, ref v2) = nodes[g.conn[i][1] as usize].as_ref().unwrap();
            let sig = make_sig(*s1, *s2, 0);

            // --- 相関係数のキャッシュチェック ---
            let node_corr = if let Some(c) = ev.corr_cache.get(&(sig, ds_idx)) {
                *c
            } else {
                // node_cache から値を取得、なければ計算
                let val = if let Some(v) = ev.node_cache.get(&sig) {
                    v.clone()
                } else {
                    let out = eml_vec(v1, v2);
                ...
                // Chatterjee + Pearson を計算してキャッシュ
                let c_val = score(&val, target);
                ev.corr_cache.insert((sig, ds_idx), c_val);
            ...
            sum_corr += node_corr;
            final_sig = sig; // 最終ノードのシグネチャを保持

            // ノードの値を保持（キャッシュヒット時もSigだけは必要）
            if nodes[abs].is_none() {
                // 実際には値が必要なのは依存関係がある場合のみだが、
                // コードを簡略化するためOptionを管理
                if let Some(v) = ev.node_cache.get(&sig) {
                    nodes[abs] = Some((sig, v.clone()));
                }
            }
        }

        let avg_intermediate_corr = sum_corr / active_count.max(1) as f64;

        // 最終出力のChatterjee（これもキャッシュから引ける）
        // 厳密には「中間は平均、最後はChatterjee単体」なので
        // 必要に応じてcorr_cacheにChatterjee単体版も入れるか、
        // あるいは中間評価にChatterjeeが含まれているのでそれを流用します。
        //let c_val = score(&nodes[N_INPUTS + MODEL_LEN - 1].as_ref().unwrap().1, target);
        total_acc += accuracy(&nodes[N_INPUTS + MODEL_LEN - 1].as_ref().unwrap().1, target);
        total_loss = (1.0 - avg_intermediate_corr) * (1.0 - total_acc);
        // 簡易的な精度指標として
    }

    let res = (total_loss / ds.n as f64, total_acc / ds.n as f64);
    ev.fitness_cache.insert(key, res);
    res
}
