...
def decode(x: list[float]) -> dict:
    ...
    for i, (name, lo, hi, init, is_int, log_scale) in enumerate(PARAM_DEFS):
        ...
        v = max(lo, min(hi, v))
        if is_int:
            v = int(round(v))  # clip は既に上で適用済み
        result[name] = v
    return result


def initial_x() -> list[float]:
    return [math.log(max(init, 1e-6)) if log_scale else float(init)
            for _, _lo, _hi, init, _is_int, log_scale in PARAM_DEFS]
...
const MIXER_WARMUP_GENERATIONS: usize = {MIXER_WARMUP_GENERATIONS};
...
def generate_rust_source(params: dict) -> str:
    ...
    src = RUST_TEMPLATE.format(
        ...
        MIXER_WARMUP_GENERATIONS=int(params["MIXER_WARMUP_GENERATIONS"]),
...
def _get_bin_name(project_dir: str) -> str:
    """Cargo.toml からバイナリ名を取得してキャッシュする。"""
    cargo_toml = os.path.join(project_dir, "Cargo.toml")
    if tomllib and os.path.exists(cargo_toml):
        with open(cargo_toml, "rb") as f:
            toml_data = tomllib.load(f)
        name = toml_data.get("package", {}).get("name")
        if name:
            return name
...
def build_and_run(
    ...
    if build_result.returncode != 0:
        ...
        return None  # objective() の acc is None チェックに委ねる

    print(f"  [trial {trial_idx}] Build OK ({build_elapsed:.1f}s). Running {eval_time}s ...", flush=True)

    binary = os.path.join(project_dir, "target", "release", _get_bin_name(project_dir))
    ...
    for line in lines:
        ...
        if m:
            try:
                ...
                pass
    best_acc = 1.0 - math.exp(best_acc / max(i, 1))
...
class CMAESSearcher:
    ...
    def run(self):
        ...
        while not self.es.stop() and self.trial_idx < self.budget:
            ...
            self._save_history()  # 世代単位で保存（trial 毎ではなく）

            iter_best_idx, iter_best_fitness = min(enumerate(fitnesses), key=lambda t: t[1])
